(function() {
    ...
    Debugger.define_module('pyimage', Game.game);
    Debugger.set_step_limit(50000)
    var doc = function(d) {
    ...
    function reset_src() {
        if (storage && storage['py_image_src']) {
            editor.setValue(storage["py_image_src"]);
        } else {
            editor.setValue('from pyimage import SimpleImage\nimage = SimpleImage("x.png")\npixel = image.getPixel(0,0)\nprint(pixel)\npixel.setRed(255)\nimage.setZoom(20)\nprint(image)');
        }
        gotoLine(0);
    }
    ...
    function run() {
        ...
        PD.sendActivity({
            action: 'run',
            event: 'python-debugger.image-manipulation',
            meta: {
                error: hist.error
            }
        });
